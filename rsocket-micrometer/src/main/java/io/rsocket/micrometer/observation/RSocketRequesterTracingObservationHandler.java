...
public class RSocketRequesterTracingObservationHandler implements TracingObservationHandler<RSocketContext> {
	...
	public RSocketRequesterTracingObservationHandler(Tracer tracer, Propagator propagator,
			Propagator.Setter<CompositeByteBuf> setter, boolean isZipkinPropagationEnabled) {
		this.tracer = tracer;
	...
	public void onStart(RSocketContext context) {
		...
		final CompositeByteBuf newMetadata = PayloadUtils.cleanTracingMetadata(payload, new HashSet<>(propagator.fields()));
	...
	private void injectDefaultZipkinRSocketHeaders(CompositeByteBuf newMetadata, TraceContext traceContext) {
		...
		if (isTraceId128Bit) {
			TracingMetadataCodec.encode128(newMetadata.alloc(), traceIds[0], traceIds[1], spanId[0],
					EncodingUtils.fromString(traceContext.parentId())[0], flags);
		}
		else {
			TracingMetadataCodec.encode64(newMetadata.alloc(), traceIds[0], spanId[0], parentSpanId[0],
					flags);
		}
	}
}
