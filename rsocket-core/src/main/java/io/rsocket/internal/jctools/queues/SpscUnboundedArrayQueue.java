...
/**
 * An SPSC array queue which starts at <i>initialCapacity</i> and grows indefinitely in linked
  ...
  final boolean offerColdPath(
    ...
    if (null == lvElement(buffer, calcElementOffset(pBufferLimit, mask))) {
      ...
      writeToQueue(buffer, v == null ? s.get() : v, pIndex, offset);
        ...
        == lvElement(buffer, calcElementOffset(pIndex + 1, mask))) { // buffer is not full
      writeToQueue(buffer, v == null ? s.get() : v, pIndex, offset);
    } else {
