...
/**
 * This is a tagging interface for the queues in this library which implement a subset of the {@link
 * Queue} interface sufficient for concurrent message passing.<br>
 * Message passing queues provide happens before semantics to messages passed through, namely that
 * writes made by the producer before offering the message are visible to the consuming thread after
 * the message has been polled out of the queue.
 *
...
public interface MessagePassingQueue<T> {
  ...
  /**
   * Called from the consumer thread subject to the restrictions appropriate to the implementation.
   * As opposed to {@link Queue#poll()} this method may return null without the queue being empty.
   *
  ...
  /**
   * Called from the consumer thread subject to the restrictions appropriate to the implementation.
   * As opposed to {@link Queue#peek()} this method may return null without the queue being empty.
   *
   ...
   * c.accept(m);</br>
   * }</br>
   * </code> There's no strong commitment to the queue being empty at the end of a drain. Called
   * from a consumer thread subject to the restrictions appropriate to the implementation.
   *
