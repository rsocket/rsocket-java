...
import static io.rsocket.util.UnsafeAccess.UNSAFE;
 ...
 * io.rsocket.util.NonBlockingHashMap} with {@link Long} keys is that it avoids the auto-boxing and
 * unboxing costs. Since auto-boxing is <em>automatic</em>, it is easy to accidentally cause
 * auto-boxing and negate the space and speed benefits.
 *
...
public class NonBlockingHashMapLong<TypeV> extends AbstractMap<Long, TypeV>
   ...
   * {@link Entry} objects with each iteration. The {@link io.rsocket.util.NonBlockingHashMap} does
   * not normally create or using {@link Entry} objects so they will be created soley to support
   * this iteration. Iterating using {@link #keySet} or {@link #values} will be more efficient. In
   * addition, this version requires <strong>auto-boxing</strong> the keys.
   */
