...
public abstract class AbstractByteBufTest {
  ...
  private ByteBuf[] innerBuffers;

  protected abstract ByteBuf[] newInnerBuffers(int capacity, int maxCapacity);

  protected abstract AbstractTupleByteBuf newBuffer(ByteBuf[] inners);

  static ByteBuf[] retainMany(ByteBuf[] buffs) {
  ...
  protected AbstractTupleByteBuf newBuffer(int capacity) {
    return newBuffer(capacity, Integer.MAX_VALUE);
  }

  protected AbstractTupleByteBuf newBuffer(int capacity, int maxCapacity) {
    innerBuffers = newInnerBuffers(capacity, maxCapacity);
    return newBuffer(innerBuffers);
  }
  ...
  public void getByteArrayBoundaryCheck3() {
    byte[] dst = new byte[4];
    CompositeByteBuf cBuffer = ByteBufAllocator.DEFAULT.compositeBuffer(innerBuffers.length);
    cBuffer.addComponents(retainMany(innerBuffers));
    cBuffer.setInt(0, 0x01020304);
    try {
  ...
  public void getByteBufferState() {
    ...
    CompositeByteBuf cbuffer =
        ByteBufAllocator.DEFAULT
            .compositeBuffer(innerBuffers.length)
            .addComponents(retainMany(innerBuffers));
    dst.position(1);
    dst.limit(3);

    cbuffer.setByte(0, (byte) 1);
    cbuffer.setByte(1, (byte) 2);
    cbuffer.setByte(2, (byte) 3);
    cbuffer.setByte(3, (byte) 4);
  ...
  private void testRandomShortAccess(boolean testBigEndian) {
    ...
    for (int i = 0; i < buffer.capacity() - 1; i += 2) {
      ...
      if (testBigEndian) {
        cBuffer.setShort(i, value);
      } else {
        cBuffer.setShortLE(i, value);
      }
    }
  ...
  private void testShortConsistentWithByteBuffer(boolean direct, boolean testBigEndian) {
    for (int i = 0; i < JAVA_BYTEBUFFER_CONSISTENCY_ITERATIONS; ++i) {
      ...
      if (testBigEndian) {
        cBuffer.setShort(bufferIndex, expected);
      } else {
        cBuffer.setShortLE(bufferIndex, expected);
      }
      javaBuffer.flip();
  ...
  private void testRandomMediumAccess(boolean testBigEndian) {
    ...
    for (int i = 0; i < buffer.capacity() - 2; i += 3) {
      ...
      if (testBigEndian) {
        cBuffer.setMedium(i, value);
      } else {
        cBuffer.setMediumLE(i, value);
      }
    }
  ...
  private void testMediumConsistentWithByteBuffer(boolean direct, boolean testBigEndian) {
    for (int i = 0; i < JAVA_BYTEBUFFER_CONSISTENCY_ITERATIONS; ++i) {
      ...
      if (testBigEndian) {
        cBuffer.setMedium(bufferIndex, expected);
      } else {
        cBuffer.setMediumLE(bufferIndex, expected);
      }
      javaBuffer.flip();
  ...
  private void testRandomIntAccess(boolean testBigEndian) {
    ...
    for (int i = 0; i < buffer.capacity() - 3; i += 4) {
      int value = random.nextInt();
      if (testBigEndian) {
        cBuffer.setInt(i, value);
      } else {
        cBuffer.setIntLE(i, value);
      }
    }
  ...
  private void testIntConsistentWithByteBuffer(boolean direct, boolean testBigEndian) {
    for (int i = 0; i < JAVA_BYTEBUFFER_CONSISTENCY_ITERATIONS; ++i) {
      ...
      if (testBigEndian) {
        cBuffer.setInt(bufferIndex, expected);
      } else {
        cBuffer.setIntLE(bufferIndex, expected);
      }
      javaBuffer.flip();
  ...
  private void testRandomLongAccess(boolean testBigEndian) {
    ...
    for (int i = 0; i < buffer.capacity() - 7; i += 8) {
      ...
      if (testBigEndian) {
        cBuffer.setLong(i, value);
      } else {
        cBuffer.setLongLE(i, value);
      }
    }
  ...
  private void testLongConsistentWithByteBuffer(boolean direct, boolean testBigEndian) {
    for (int i = 0; i < JAVA_BYTEBUFFER_CONSISTENCY_ITERATIONS; ++i) {
      ...
      if (testBigEndian) {
        cBuffer.setLong(bufferIndex, expected);
      } else {
        cBuffer.setLongLE(bufferIndex, expected);
      }
      javaBuffer.flip();
  ...
  private void testRandomFloatAccess(boolean testBigEndian) {
    ...
    for (int i = 0; i < buffer.capacity() - 7; i += 8) {
      ...
      if (testBigEndian) {
        cBuffer.setFloat(i, value);
      } else {
        cBuffer.setFloatLE(i, value);
      }
    }
  ...
  private void testRandomDoubleAccess(boolean testBigEndian) {
    ...
    for (int i = 0; i < buffer.capacity() - 7; i += 8) {
      ...
      if (testBigEndian) {
        cBuffer.setDouble(i, value);
      } else {
        cBuffer.setDoubleLE(i, value);
      }
    }
  ...
  public void testSetZero() {
    CompositeByteBuf cBuffer = ByteBufAllocator.DEFAULT.compositeBuffer(innerBuffers.length);
    cBuffer.addComponents(retainMany(innerBuffers));
    while (cBuffer.isWritable()) {
      cBuffer.writeByte((byte) 0xFF);
    }

    for (int i = 0; i < buffer.capacity(); ) {
      int length = Math.min(buffer.capacity() - i, random.nextInt(32));
      cBuffer.setZero(i, length);
      i += length;
    }
  ...
  public void testSequentialByteAccess() {
    ...
    for (int i = 0; i < buffer.capacity(); i++) {
      byte value = (byte) random.nextInt();
      assertEquals(i, cBuffer.writerIndex());
      assertTrue(cBuffer.isWritable());
      cBuffer.writeByte(value);
    }

    assertEquals(0, buffer.readerIndex());
    assertEquals(buffer.capacity(), cBuffer.writerIndex());
    assertFalse(cBuffer.isWritable());
  ...
  private void testSequentialShortAccess(boolean testBigEndian) {
    ...
    for (int i = 0; i < buffer.capacity(); i += 2) {
      ...
      assertTrue(cBuffer.isWritable());
      if (testBigEndian) {
        cBuffer.writeShort(value);
      } else {
        cBuffer.writeShortLE(value);
      }
    }
  ...
  private void testSequentialMediumAccess(boolean testBigEndian) {
    ...
    for (int i = 0; i < buffer.capacity() / 3 * 3; i += 3) {
      ...
      assertTrue(cBuffer.isWritable());
      if (testBigEndian) {
        cBuffer.writeMedium(value);
      } else {
        cBuffer.writeMediumLE(value);
      }
    }
    buffer = newBuffer(innerBuffers).setIndex(cBuffer.readerIndex(), cBuffer.writerIndex());
  ...
  private void testSequentialUnsignedMediumAccess(boolean testBigEndian) {
    ...
    assertEquals(0, buffer.readerIndex());
    assertEquals(buffer.capacity() / 3 * 3, cBuffer.writerIndex());
    assertEquals(buffer.capacity() % 3, cBuffer.writableBytes());
    ...
    for (int i = 0; i < buffer.capacity() / 3 * 3; i += 3) {
      ...
      assertTrue(buffer.isReadable());
      if (testBigEndian) {
        assertEquals("Incorrect value at index=" + i, value, buffer.readUnsignedMedium());
      } else {
        assertEquals("Incorrect value at index=" + i, value, buffer.readUnsignedMediumLE());
      }
    }
  ...
  private void testSequentialIntAccess(boolean testBigEndian) {
    ...
    for (int i = 0; i < buffer.capacity(); i += 4) {
      ...
      assertTrue(cBuffer.isWritable());
      if (testBigEndian) {
        cBuffer.writeInt(value);
      } else {
        cBuffer.writeIntLE(value);
      }
    }
  ...
  private void testSequentialLongAccess(boolean testBigEndian) {
    ...
    for (int i = 0; i < buffer.capacity(); i += 8) {
      ...
      assertTrue(cBuffer.isWritable());
      if (testBigEndian) {
        cBuffer.writeLong(value);
      } else {
        cBuffer.writeLongLE(value);
      }
    }

    buffer.setIndex(cBuffer.readerIndex(), cBuffer.writerIndex());
  ...
  public void testByteArrayTransfer() {
    ...
    for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
      ...
      cbuffer.setBytes(i, value, random.nextInt(BLOCK_SIZE), BLOCK_SIZE);
    }
  ...
  public void testSequentialByteArrayTransfer1() {
    ...
    for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
      random.nextBytes(value);
      assertEquals(0, cbuffer.readerIndex());
      assertEquals(i, cbuffer.writerIndex());
      cbuffer.writeBytes(value);
    }
  ...
  public void testSequentialByteArrayTransfer2() {
    ...
    for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
      ...
      cbuffer.writeBytes(value, readerIndex, BLOCK_SIZE);
    }
  ...
  public void testSequentialHeapBufferTransfer1() {
    ...
    for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
      ...
      assertEquals(0, cbuffer.readerIndex());
      assertEquals(i, cbuffer.writerIndex());
      cbuffer.writeBytes(value, random.nextInt(BLOCK_SIZE), BLOCK_SIZE);
      assertEquals(0, value.readerIndex());
  ...
  public void testSequentialByteBufferTransfer() {
    ...
    for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
      ...
      value.clear().position(valueOffset).limit(valueOffset + BLOCK_SIZE);
      buffer.readBytes(value);
      assertEquals("" + i, valueOffset + BLOCK_SIZE, value.position());
      for (int j = valueOffset; j < valueOffset + BLOCK_SIZE; j++) {
        assertEquals(expectedValue.get(j), value.get(j));
      }
    }
  ...
  public void testSequentialSlice1() {
    ...
    for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
      ...
      try {
        ...
        cbuffer.setByte(0, (byte) (actualValue.getByte(0) + 1));
      }
  ...
  public void testStreamTransfer1() throws Exception {
    ...
    for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
      ...
      assertEquals(BLOCK_SIZE, cbuffer.setBytes(i, in, BLOCK_SIZE));
      assertEquals(-1, cbuffer.setBytes(i, in, 0));
    }
  ...
  public void testStreamTransfer2() throws Exception {
    ...
    for (int i = 0; i < buffer.capacity() - BLOCK_SIZE + 1; i += BLOCK_SIZE) {
      ByteArrayInputStream in = new ByteArrayInputStream(expected, i, BLOCK_SIZE);
      assertEquals(i, cbuffer.writerIndex());
      cbuffer.writeBytes(in, BLOCK_SIZE);
      assertEquals(i + BLOCK_SIZE, cbuffer.writerIndex());
    }
  ...
  public void testCopy() {
    ...
    cbuffer.setByte(readerIndex, (byte) (buffer.getByte(readerIndex) + 1));
  ...
  public void testDuplicate() {
    ...
    //        duplicate.setByte(duplicate.readerIndex(), (byte)
    // (duplicate.getByte(duplicate.readerIndex()) + 1));
    //        assertEquals(buffer.getByte(readerIndex), duplicate.getByte(duplicate.readerIndex()));
    cBuffer.release();
  }
  ...
  public void testCompareTo2() {
    ...
    ByteBuf buf1 = newBuffer(4).order(ByteOrder.LITTLE_ENDIAN);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(bytes)
        .release();
    ByteBuf buf2 = newBuffer(4).order(ByteOrder.LITTLE_ENDIAN);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(bytesReversed)
        .release();
    ByteBuf buf3 = newBuffer(4).order(ByteOrder.BIG_ENDIAN);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(bytes)
        .release();
    ByteBuf buf4 = newBuffer(4).order(ByteOrder.BIG_ENDIAN);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(bytesReversed)
  ...
  public void testToString() {
    ...
    CompositeByteBuf cbuffer = ByteBufAllocator.DEFAULT.compositeBuffer(innerBuffers.length);
    cbuffer.addComponents(retainMany(innerBuffers));
    cbuffer.writeBytes(copied);
    buffer.writerIndex(cbuffer.writerIndex());
  ...
  public void testToStringMultipleThreads() throws Throwable {
    byte[] bytes = "Hello, World!".getBytes(CharsetUtil.ISO_8859_1);
    buffer = newBuffer(bytes.length);
    CompositeByteBuf cBuffer = ByteBufAllocator.DEFAULT.compositeBuffer(innerBuffers.length);
    cBuffer.addComponents(retainMany(innerBuffers));
  ...
  public void testIndexOf() {
    CompositeByteBuf cbuffer =
        ByteBufAllocator.DEFAULT
            .compositeBuffer(innerBuffers.length)
            .addComponents(retainMany(innerBuffers));
    cbuffer.writeByte((byte) 1);
    cbuffer.writeByte((byte) 2);
    cbuffer.writeByte((byte) 3);
    cbuffer.writeByte((byte) 2);
    cbuffer.writeByte((byte) 1);
  ...
  public void testHashCode() {
    ...
    cbuffer.clear();
    cbuffer.writeBytes(elemA.duplicate());
    buffer.setIndex(cbuffer.readerIndex(), cbuffer.writerIndex());
    ...
    cbuffer.clear();
    cbuffer.writeBytes(elemB.duplicate());
    buffer.setIndex(cbuffer.readerIndex(), cbuffer.writerIndex());
  ...
  private void testInternalNioBuffer(int a) {
    ...
    cbuffer.writeBytes(data);

    for (ByteBuf innerBuffer : innerBuffers) {
      int innerSize = innerBuffer.capacity();
      buf = buffer.internalNioBuffer(buffer.readerIndex(), innerSize);
      assertEquals(innerSize, buf.remaining());

      for (int i = 0; i < innerSize; i++) {
        assertEquals(data[buffer.readerIndex() + i], buf.get());
      }
      assertFalse(buf.hasRemaining());
      buffer.skipBytes(innerSize);
    }
    try {
      for (ByteBuf inner : innerBuffers) {
        if (inner.capacity() > 0 && inner.capacity() != a) {
          buffer.internalNioBuffer(0, a);
          fail();
        }
      }
  ...
  public void testNioBufferExposeOnlyRegion() {
    final ByteBuf buffer = newBuffer(8);
    final CompositeByteBuf cbuffer =
        ByteBufAllocator.DEFAULT
            .compositeBuffer(innerBuffers.length)
            .addComponents(retainMany(innerBuffers));
  ...
  private void testSetGetCharSequence(Charset charset) {
    ...
    CompositeByteBuf cbuf = ByteBufAllocator.DEFAULT.compositeBuffer(innerBuffers.length);
    cbuf.addComponents(retainMany(innerBuffers));
    CharBuffer sequence = CharsetUtil.US_ASCII.equals(charset) ? ASCII_CHARS : EXTENDED_ASCII_CHARS;
    int bytes = cbuf.setCharSequence(1, sequence, charset);
  ...
  private void testWriteReadCharSequence(Charset charset) {
    ...
    CompositeByteBuf cBuffer = ByteBufAllocator.DEFAULT.compositeBuffer(innerBuffers.length);
    cBuffer.addComponents(retainMany(innerBuffers));
    cBuffer.writerIndex(1);
    int bytes = cBuffer.writeCharSequence(sequence, charset);
    buf.readerIndex(1);
  ...
  public void testRetainedSliceAndRetainedDuplicateContentIsExpected() {
    ByteBuf[] innerBuffers = newInnerBuffers(8, Integer.MAX_VALUE);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(new byte[] {1, 2, 3, 4, 5, 6, 7, 8})
        .release();
    ByteBuf buf = newBuffer(innerBuffers);

    innerBuffers = newInnerBuffers(6, Integer.MAX_VALUE);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(new byte[] {2, 3, 4, 5, 6, 7})
        .release();
    ByteBuf expected1 = newBuffer(innerBuffers);

    innerBuffers = newInnerBuffers(5, Integer.MAX_VALUE);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(new byte[] {3, 4, 5, 6, 7})
        .release();
    ByteBuf expected2 = newBuffer(innerBuffers);

    innerBuffers = newInnerBuffers(4, Integer.MAX_VALUE);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(new byte[] {4, 5, 6, 7})
        .release();
    ByteBuf expected3 = newBuffer(innerBuffers);

    innerBuffers = newInnerBuffers(3, Integer.MAX_VALUE);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(new byte[] {5, 6, 7})
        .release();
    ByteBuf expected4 = newBuffer(innerBuffers);
    ...
    for (ByteBuf innerBuffer : innerBuffers) {
      if (innerBuffer != Unpooled.EMPTY_BUFFER) {
        assertEquals(0, innerBuffer.refCnt());
      }
    }
  }
  ...
  public void testRetainedDuplicateAndRetainedSliceContentIsExpected() {
    ...
    ByteBufAllocator.DEFAULT
        ...
        .writeBytes(new byte[] {5, 6, 7})
        .release();
    for (int i = innerBuffers.length - 1; i >= 0; i--) {
      ByteBuf innerBuffer = innerBuffers[i];
      if (innerBuffer.capacity() > 0) {
        innerBuffer.writerIndex(innerBuffer.writerIndex() - 1);
        break;
      }
    }
    ByteBuf expected3 = newBuffer(innerBuffers);
  ...
  private void testSliceContents(boolean retainedSlice) {
    ...
    ByteBuf expected = newBuffer(3);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(new byte[] {4, 5, 6})
        .release();
  ...
  private void testSliceReleaseOriginal(boolean retainedSlice1, boolean retainedSlice2) {
    ...
    ByteBuf expected1 = newBuffer(3);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(new byte[] {6, 7, 8})
        .release();

    ByteBuf expected2 = newBuffer(2);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
  ...
  private void testMultipleLevelRetainedSliceWithNonRetained(boolean doSlice1, boolean doSlice2) {
    ...
    ByteBuf expected3 = newBuffer(2);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(new byte[] {4, 5})
        .release();

    ByteBuf expected4SliceSlice = newBuffer(1);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
    ...
    ByteBuf expected4DupSlice = newBuffer(1);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
  ...
  private void testDuplicateReleaseOriginal(
    ...
    ByteBuf expected = newBuffer(8);
    ByteBufAllocator.DEFAULT
        .compositeBuffer(innerBuffers.length)
        .addComponents(retainMany(innerBuffers))
        .writeBytes(buf, buf.readerIndex(), buf.readableBytes())
        .release();
  ...
  public void testReadBytesAndWriteBytesWithFileChannel() throws IOException {
    ...
    try {
      ...
      CompositeByteBuf cbuffer2 =
          ByteBufAllocator.DEFAULT
              .compositeBuffer(innerBuffers.length)
              .addComponents(retainMany(innerBuffers));
      buffer2.resetReaderIndex();
      buffer2.resetWriterIndex();
      buffer2.setIndex(cbuffer2.readerIndex(), cbuffer2.writerIndex());

      int oldWriterIndex = buffer2.writerIndex();
      assertEquals(len, cbuffer2.writeBytes(channel, 10, len));
      assertEquals(channelPosition, channel.position());
      assertEquals(oldWriterIndex + len, buffer2.writerIndex(cbuffer2.writerIndex()).writerIndex());
    ...
    try {
      ...
      CompositeByteBuf cbuffer2 =
          ByteBufAllocator.DEFAULT
              ...
              .addComponents(retainMany(innerBuffers));
      int oldWriterIndex = buffer2.writerIndex();
      assertEquals(cbuffer2.setBytes(oldWriterIndex, channel, 10, len), len);
      assertEquals(channelPosition, channel.position());
      cbuffer2.release();
  ...
  @Test(expected = ReadOnlyBufferException.class)
  public void testCapacityReadOnly() {
    buffer.capacity(14);
  }
}
