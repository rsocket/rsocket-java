    ...
    public void testPing() {
       ...
       // ctx.publicationConnectionTimeout(TimeUnit.MINUTES.toNanos(5));
        ...
        ctx.availableImageHandler(image -> {
            System.out.println("name image subscription => "
        ...
        ctx.unavailableImageHandler(image -> {
            System.out.println("=== unavailable image name image subscription => "
                + image.subscription().channel()
                + " streamid => "
                + image.subscription().streamId()
                + " registrationid => " + image.subscription().registrationId());
...
*/
        ...
        Subscription serverSubscription = aeron.addSubscription("aeron:udp?endpoint=localhost:39791", Constants.SERVER_STREAM_ID);
        System.out.println("serverSubscription registration id => " + serverSubscription.registrationId());

        Publication serverPublication = aeron.addPublication("aeron:udp?endpoint=localhost:39790", Constants.CLIENT_STREAM_ID);

        Subscription clientSubscription = aeron.addSubscription("aeron:udp?endpoint=localhost:39790", Constants.CLIENT_STREAM_ID);

        System.out.println("clientSubscription registration id => " + clientSubscription.registrationId());
        Publication clientPublication = aeron.addPublication("aeron:udp?endpoint=localhost:39791", Constants.SERVER_STREAM_ID);
...
*/
        ...
        AeronChannel serverChannel = new AeronChannel("server", serverPublication,  serverSubscription, serverLoop, clientPublication.sessionId());
        ...
        AeronChannel clientChannel = new AeronChannel("client", clientPublication, clientSubscription, clientLoop, serverPublication.sessionId());
