...
public abstract class AbstractJacksonCodec implements Codec {
    ...
    private static final ThreadLocal<DirectBufferOutputStream> directOutWrappers =
            ThreadLocal.withInitial(DirectBufferOutputStream::new);
    ...
    public <T> void encodeTo(MutableDirectBuffer buffer, T toEncode, int offset) {
        DirectBufferOutputStream stream = directOutWrappers.get();
        stream.wrap(buffer, offset, buffer.capacity());
