...
public class MySubscriber<T> extends TestSubscriber<T> {
    ...
    /**
     * this will be locked everytime we await at most some number of myValues, the await will always be with a timeout
     * After the timeout, we look at the value inside the countdown latch to make sure we counted down the
     * number of myValues we expected
     */
        ...
        for (int i = 0; i < values.size(); i++) {
            ...
            if (!Objects.equals(u, v)) {
                myFail(prefix + "Values at position " + i + " differ; Expected: "
                        + valueAndClass(u) + ", Actual: " + valueAndClass(v));
